...
public class MoneyList {
	...
	public void printReverseOrder (MoneyNode p) {
		//base case: empty list 
		if (p==null)
			return;
		
		printReverseOrder (p.next);
		System.out.println(p.data);
	}
	/**
	 * 
	...
	public double sumMoney(MoneyNode p) {
		...
		//base case: empty list 
				...
				if (p.data instanceof Bill) // Bill type
					return ((Bill)p.data).getValue() + sumMoney(p.next); // dollar amount
				else // coin type 0.25
					return ((Coin)p.data).getValue()/100.0 + sumMoney(p.next); // cents amount
...
public double sumQuarter(MoneyNode p) {
		...
		//base case: empty list 
				...
				if (p.data instanceof Quarter) // Bill type
					return ((Quarter)p.data).getValue()/100.0 + sumQuarter(p.next); // cents amount
